...
Transform::Transform() : Node("transform_node"),
    ...
    // 坐标系参数：默认值设为常用的"base_link"和"lidar_link"
this->declare_parameter("parents_frame", "base_link");
this->declare_parameter("child_frame", "lidar_link");
// 位置参数（x/y/z）：默认值设为0.0（单位：米）
this->declare_parameter("x", 0.0);
this->declare_parameter("y", 0.0);
this->declare_parameter("z", 0.0);
// 姿态参数（roll/pitch/yaw）：默认值设为0.0（单位：弧度，0表示无旋转）
